...
import { MessageSquare, ChevronLeft, Send, User } from "lucide-react";
...
export const ChatDrawer = ({ open, onClose }: ChatDrawerProps) => {
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="space-y-3 flex-grow overflow-auto">
        {mockDoctors.map((doctor, index) => (
          <Card 
            key={doctor.id} 
            className="bg-white cursor-pointer hover:shadow-md transition-all hover:scale-[1.02]"
  ...
  return (
    <div className={`fixed inset-0 z-50 bg-black/20 transition-all ${open ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
      <div 
        className={`fixed bottom-0 left-0 right-0 w-full max-w-md mx-auto bg-white rounded-t-2xl h-3/4 p-5 shadow-lg
          transition-transform transform duration-300 ${open ? 'translate-y-0' : 'translate-y-full'}`}
      >
